...
export async function getStockQuotes(symbols: string[]): Promise<StockQuote[]> {
    if (!FINNHUB_API_KEY || symbols.length === 0) {
        return symbols.map(s => ({ symbol: s, price: null, change: null, changePercent: null, marketCap: null, peRatio: null }));
    }
        ...
        symbols.map(async (symbol) => {
            try {
                const [quoteRes, profileRes] = await Promise.all([
                    fetch(`${BASE_URL}/quote?symbol=${symbol}&token=${FINNHUB_API_KEY}`, { next: { revalidate: 60 } }),
                    fetch(`${BASE_URL}/stock/metric?symbol=${symbol}&metric=all&token=${FINNHUB_API_KEY}`, { next: { revalidate: 3600 } }),
                ]);

                const quote = quoteRes.ok ? await quoteRes.json() : {};
                const metrics = profileRes.ok ? await profileRes.json() : {};
